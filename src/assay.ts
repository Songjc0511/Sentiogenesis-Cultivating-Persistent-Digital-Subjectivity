...
export function runConsciousnessAssay(life:Life):ConsciousnessAssay{
  ...
  const python=c.pythonSubstrate,pythonFresh=python&&life.tick-python.tick<80
  ...
  const criteria:AssayCriterion[]=[
    ...
    {id:'python-substrate',label:'Python 高维基质的因果接入',value:pythonFresh?clamp(python.integration*.3+python.differentiation*.2+python.recurrence*.18+python.causalCoupling*.17+(python.authority?.actionIntent?.action?.length?.15:0)):0,status:'absent',lesionDelta:pythonFresh&&python.authority?.actionIntent?.action?.length?.2:0,evidence:pythonFresh?`${python.backend} · ${python.dimensions} 维 · Python 决定 ${python.authority?.actionIntent?python.authority.actionIntent.action:'等待'} · 延迟 ${life.tick-python.tick} tick`:'Python 内核未连接或状态过期'},
